...
# Email gate: compiled SPA (Caddy / NGINX serves this for /)
cp -r "$REPO_ROOT/artifacts/email-gate/dist/public" "$OUT/email-gate/public"

# Deploy configs (Caddyfile, nginx.conf, systemd service)
mkdir -p "$OUT/deploy"
cp -r "$REPO_ROOT/deploy/." "$OUT/deploy/"
...
echo ""
echo "Then follow CADDY_DEPLOYMENT.md (Caddy) or DEPLOYMENT.md (NGINX)."
